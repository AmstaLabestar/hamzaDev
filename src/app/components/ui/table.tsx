...
function TableBody({ className, ...props }: React.ComponentProps<"tbody">) {
  return (
    <tbody
      ...
      className={cn("[&_tr:last-child]:border-0 [&_tr]:border-border/60", className)}
      {...props}
    />
  );
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-muted/45 data-[state=selected]:bg-muted border-b transition-[background-color,transform] duration-200",
        className,
      )}
      {...props}
    />
